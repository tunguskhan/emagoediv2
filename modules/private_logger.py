...
def log(img, metadata, metadata_parser: MetadataParser | None = None, output_format=None) -> str:
    ...
    if output_format == OutputFormat.PNG.value:
        if parsed_parameters != '':
            ...
            pnginfo.add_text('emagoediv_scheme', metadata_parser.get_scheme().value)
        else:
            pnginfo = None
    ...
    begin_part = f"<!DOCTYPE html><html><head><title>emagoediv Log {date_string}</title>{css_styles}</head><body>{js}<p>emagoediv Log {date_string} (private)</p>\n<p>Metadata is embedded if enabled in the config or developer debug mode. You can find the information for each image in line Metadata Scheme.</p><!--emagoediv-log-split-->\n\n"
    end_part = f'\n<!--emagoediv-log-split--></body></html>'

    middle_part = log_cache.get(html_name, "")

    if middle_part == "":
        if os.path.exists(html_name):
            existing_split = open(html_name, 'r', encoding='utf-8').read().split('<!--emagoediv-log-split-->')

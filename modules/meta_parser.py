...
class A1111MetadataParser(MetadataParser):
    ...
    def parse_json(self, metadata: str) -> dict:
        ...
        *lines, lastline = metadata.strip().split("\n")
        ...
        for line in lines:
            line = line.strip()
            if line.startswith(f"{self.emagoediv_to_a1111['negative_prompt']}:"):
                done_with_prompt = True
                line = line[len(f"{self.emagoediv_to_a1111['negative_prompt']}:"):].strip()
            if done_with_prompt:
        ...
        for k, v in re_param.findall(lastline):
            try:
                ...
                if m is not None:
                    ...
                    data[list(self.emagoediv_to_a1111.keys())[list(self.emagoediv_to_a1111.values()).index(k)]] = v
            except Exception:
        ...
        if 'raw_prompt' in data:
            ...
            if metadata_prompt != raw_prompt and modules.sdxl_styles.emagoediv_expansion not in found_styles:
                found_styles.append(modules.sdxl_styles.emagoediv_expansion)
    ...
    def parse_string(self, metadata: dict) -> str:
        ...
        generation_params = {
            self.emagoediv_to_a1111['steps']: self.steps,
            self.emagoediv_to_a1111['sampler']: sampler,
            self.emagoediv_to_a1111['seed']: data['seed'],
            self.emagoediv_to_a1111['resolution']: f'{width}x{height}',
            self.emagoediv_to_a1111['guidance_scale']: data['guidance_scale'],
            self.emagoediv_to_a1111['sharpness']: data['sharpness'],
            self.emagoediv_to_a1111['adm_guidance']: data['adm_guidance'],
            self.emagoediv_to_a1111['base_model']: Path(data['base_model']).stem,
            self.emagoediv_to_a1111['base_model_hash']: self.base_model_hash,

            self.emagoediv_to_a1111['performance']: data['performance'],
            self.emagoediv_to_a1111['scheduler']: scheduler,
            # workaround for multiline prompts
            self.emagoediv_to_a1111['raw_prompt']: self.raw_prompt,
            self.emagoediv_to_a1111['raw_negative_prompt']: self.raw_negative_prompt,
        }

        if self.refiner_model_name not in ['', 'None']:
            generation_params |= {
                self.emagoediv_to_a1111['refiner_model']: self.refiner_model_name,
                self.emagoediv_to_a1111['refiner_model_hash']: self.refiner_model_hash
            }

        for key in ['adaptive_cfg', 'overwrite_switch', 'refiner_swap_method', 'freeu']:
            if key in data:
                generation_params[self.emagoediv_to_a1111[key]] = data[key]

        if len(self.loras) > 0:
            lora_hashes = []
            lora_weights = []
            for index, (lora_name, lora_weight, lora_hash) in enumerate(self.loras):
                # workaround for emagoediv not knowing LoRA name in LoRA metadata
            ...
            generation_params[self.emagoediv_to_a1111['lora_hashes']] = lora_hashes_string
            generation_params[self.emagoediv_to_a1111['lora_weights']] = lora_weights_string

        generation_params[self.emagoediv_to_a1111['version']] = data['version']

        if modules.config.metadata_created_by != '':
            generation_params[self.emagoediv_to_a1111['created_by']] = modules.config.metadata_created_by
...
class emagoedivMetadataParser(MetadataParser):
    def get_scheme(self) -> MetadataScheme:
        return MetadataScheme.emagoediv
...
def get_metadata_parser(metadata_scheme: MetadataScheme) -> MetadataParser:
    match metadata_scheme:
        case MetadataScheme.emagoediv:
            return emagoedivMetadataParser()
...
def read_info_from_image(filepath) -> tuple[str | None, MetadataScheme | None]:
    ...
    metadata_scheme = items.pop('emagoediv_scheme', None)
    ...
    try:
        ...
        if isinstance(parameters, dict):
            metadata_scheme = MetadataScheme.emagoediv
...
def get_exif(metadata: str | None, metadata_scheme: str):
    ...
    # 0x0131 = Software
    exif[0x0131] = 'emagoediv v' + emagoediv_version.version
    # 0x927C = MakerNote

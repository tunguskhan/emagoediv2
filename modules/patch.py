...
def calculate_weight_patched(self, patches, weight, key):
    for p in patches:
        ...
        if patch_type == "diff":
            ...
            if alpha != 0.0:
                if w1.shape != weight.shape:
                    print("WARNING SHAPE MISMATCH {} emagoediv WEIGHT NOT MERGED {} != {}".format(key, w1.shape, weight.shape))
                else:
                    weight += alpha * ldm_patched.modules.model_management.cast_to_device(w1, weight.device, weight.dtype)
...
def patched_load_models_gpu(*args, **kwargs):
    ...
    if moving_time > 0.1:
        print(f'[emagoediv Model Management] Moving model(s) has taken {moving_time:.2f} seconds')
    return y
            ...
            for path in list(args) + list(kwargs.values()):
                if isinstance(path, str):
                    if os.path.exists(path):
                        ...
                        if os.path.exists(path):
                            os.remove(path)
                        exp += f'emagoediv has tried to move the corrupted file to {corrupted_backup_file} \n'
                        exp += f'You may try again now and emagoediv will download models again. \n'
